...
class SpikEmo(nn.Module):
    ...
    def forward(self, texts, audios, visuals, speaker_masks, utterance_masks, padded_labels):
        ...
        if self.dataset == 'IEMOCAP':
            text_context = self.text_dialoguernn(text_features, speaker_masks, utterance_masks)
            text_features = 0.3 * text_context + 0.7 * text_features

        audio_features = self.audio_fc(audios)
        audio_context = self.audio_dialoguernn(audio_features, speaker_masks, utterance_masks)
        audio_features = 0.3 * audio_context + 0.7 * audio_features


        visual_features = self.visual_fc(visuals)
        visual_context = self.visual_dialoguernn(visual_features, speaker_masks, utterance_masks)
        visual_features = 0.3 * visual_context + 0.7 * visual_features
